...
- **Problem**: `VoteSMTInputsBuilder` generated wrong signal names for the vote_smt circuit (`citizenship`, `currentDate`, `proposalEventId`, `secretKey`, `identityCreationTimestamp`, `proposalId`). The witness calculator rejected with "Error loading signal citizenship: Too many values". **Root cause**: The vote_smt circuit is a Semaphore-style SMT voting circuit. Its actual signals are: `root`, `nullifierHash`, `nullifier`, `vote`, `secret`, `pathElements[20]`, `pathIndices[20]`. The identity fields (citizenship, timestamps) are committed into the SMT leaf during registration — the voting circuit only proves you know the secret + Merkle path. **Discovery method**: Found `input.json` in the original FreedomToolAndroid repo (`app/src/main/res/raw/input.json`) which contains the correct signal names. **Lesson**: When working with compiled circuit binaries (`.dat`), always check the upstream repo for sample inputs or circuit source rather than guessing signal names from contract ABI fields.
- **Problem**: Advancing Hardhat node time with `evm_setNextBlockTimestamp` past proposal end dates makes all proposals status=3 (ENDED). Can't go backwards — Hardhat requires timestamps to be >= last block. **Solution**: Don't advance time beyond the shortest proposal's end date. If you overshoot, restart the Hardhat node (`pkill -f "hardhat node"`), redeploy, and re-seed. The seed script uses `now = Date.now()` for real-world time, keeping proposals synchronized with the device clock.
- **Problem**: Seed script `seed-local.ts` used `now = realNow` (device wall-clock time) even when the chain had been manually advanced past that. Fresh proposals got start timestamps in the past relative to the chain → immediately ended. **Solution**: Fixed seed script to use `now = Math.max(realNow, chainTime)` so proposals are always created relative to whichever clock is ahead.
- **Problem**: After restarting Hardhat node (fresh chain), the Android app still showed "Enrolled" (already voted) on proposals because `SecureSharedPrefs.getVoteResult(proposalId)` persists across chain resets. The app stores vote results by proposal ID in SharedPreferences, and the new chain reuses the same IDs. **Solution**: Clear app data with `adb shell pm clear org.iranUnchained.local` before testing on a fresh chain. This resets SharedPreferences, triggering the full onboarding flow (welcome → PIN setup → LocalDevSeeder identity creation).
- **Problem**: Tried to use Chrome browser automation MCP tools (`mcp__claude-in-chrome__*`) to interact with the Android emulator. These tools control a Chrome browser, not an Android emulator. **Solution**: Always use `adb` commands for Android emulator interaction: `adb shell input tap X Y` for taps, `adb exec-out screencap -p` for screenshots, `adb shell uiautomator dump` for UI hierarchy. Use `adb shell input swipe` for swipes/scrolls.
- **Problem**: Screenshot coordinates from the image viewer don't match device coordinates. The image note says "Multiply coordinates by 1.20 to map to original image." **Solution**: Always use `adb shell uiautomator dump` to get exact device-pixel bounds for UI elements, then compute center coordinates from bounds. Don't estimate from visual screenshot positions.
